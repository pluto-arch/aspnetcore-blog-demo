...
namespace Pluto.BlogCore.Application.Behaviors
{
    ...
    public class LoggingBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse>
    {
        ...
        {
            _logger.LogInformation(_eventIdProvider.EventId, "Exec {@command}：{@Command}",request.GetType().Name, request);
            var response = await next();
            _logger.LogInformation(_eventIdProvider.EventId, "Exec {@command} result: {@Response}",request.GetType().Name, response);
            return response;
        }
    }
}